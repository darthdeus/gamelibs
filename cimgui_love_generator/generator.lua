...
local f = assert(io.open("cimgui_love_generator/out/cdef.lua", "w"))
...
local f = assert(io.open("cimgui_love_generator/base.lua", "r"))
wrap[1] = f:read("*all")
f:close()
...
-- ImTextureRef coercion was LÖVE-specific (auto-converted love.Texture
-- objects). gamelibs targets non-LÖVE consumers; callers pass a real
-- ImTextureRef (e.g. via ImTextureRef_TextureID) so emit no conversion.
...
for _, name in ipairs(sorted_entries(classes)) do
    ...
    for _, m in ipairs(class.methods) do
        ...
        for i, arg in ipairs(m.in_argsT) do
            if arg.type == "ImDrawCallback" then
                wrap[#wrap + 1] = templates.drawcallback:gsub("&arg&", string.format("i%d", i))
            elseif arg.type == "ImTextureRef" then
...
local f = assert(io.open("cimgui_love_generator/out/wrap.lua", "w"))
f:write(table.concat(wrap, "\n"))
f:close()

local f = assert(io.open("cimgui_love_generator/out/ignored_defaults.txt", "w"))
f:write(table.concat(ignored_defaults, "\n"))
f:close()

local f = assert(io.open("cimgui_love_generator/out/ignored_out_arg.txt", "w"))
f:write(table.concat(ignored_out_arg, "\n"))
f:close()

local f = assert(io.open("cimgui_love_generator/out/overloads.txt", "w"))
...
local f = assert(io.open("cimgui_love_generator/out/enums.lua", "w"))
